...
| **AI** | OpenAI API (gpt-4o-mini) | Yalnızca F3 için — JSON constraint ile, key kaynağı aşağıda |
...
### F3 API Çağrı Stratejisi
İki opsiyon, ikisi de geçerli — takım hackathon başında bir kez karar verir, sonra değişmez.

**Opsiyon 1 — n8n webhook (tercih edilir):**
```
Frontend → n8n webhook (/webhook/f3-rehber, POST: profil + tesis JSON)
        → n8n: OpenAI node (sistem promptu + JSON kuralı + red flag filtresi)
        → Frontend: kişiselleştirilmiş rehber JSON
```
Avantaj: (1) Brief'in n8n kredisinden karşılanır, ayrı OpenAI key tedariki gerekmez. (2) Key tarayıcıda görünmez, repo'ya sızmaz. (3) Sistem promptu + JSON kuralı + red flag tek noktada (n8n workflow) tutulur — frontend bypass edilemez.

**Opsiyon 2 — Frontend direkt çağrı:**
Vite env variable (`VITE_OPENAI_KEY`) ile OpenAI doğrudan çağrılır. `.env.local` gitignore'da. Sistem promptu + red flag listesi frontend kodunda tutulur.
Avantaj: Hızlı kurulum, az hareketli parça, n8n öğrenme yükü yok.
Dezavantaj: Key build çıktısında görünür — demo Vercel deploy'u public ise risk taşır.

İki opsiyonda da fallback aynı: webhook/API başarısız olursa statik şablon devreye girer, demo durmaz.
...
**Council Kararı:**
Sayısal yüzde yok. Boyutlar etiketle konuşur:
`Giriş: Doğrulanmış ✅` / `Soyunma: Bilgi yok ⚠️` / `Ekipman: Mevcut değil ❌`
...
**Council Kararı:**
Renk hesaplama kural tabanlıdır, LLM kullanılmaz. "Community-sourced" çerçevesi kullanılır — "resmi denetim" değil, "topluluk verisi" dili.
